  ...
  return (
    <div className=' pt-20 sm:pt-[20%]  pr-40 pl-5 md:px-10 absolute text-white bg-gradient-to-r from-black w-[100%] aspect-video'>
      ...
      <div className='my-5 flex md:block'>
        <button className='bg-white text-black p-2 text-sm  md:px-10  rounded-sm mr-2 hover:bg-opacity-90'><Link to={`/movie/${id}`}>▶️ Play</Link></button>
        <button className='bg-gray-500 text-white p-2 text-sm md:px-10  rounded-sm  bg-opacity-50 hover:bg-opacity-90'><Link to={`/movie/${id}`}>ℹ️ More Info </Link> </button>
      </div>
    </div>
  )
}
